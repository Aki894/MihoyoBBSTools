...
push_config = {
    ...
    'DEER_KEY2': '',                    # PushDeer2 的 PUSHDEER_KEY
...
def pushdeer2(title: str, content: str) -> None:
    """
    通过PushDeer 推送消息
    """
    if not push_config.get("DEER_KEY2"):
        print("PushDeer2 服务的 DEER_KEY 未设置!!\n取消推送")
        return
    print("PushDeer2 服务启动")
    data = {
        "text": title,
        "desp": content,
        "type": "markdown",
        "pushkey": push_config.get("DEER_KEY2"),
    }
    url = "https://api2.pushdeer.com/message/push"
    if push_config.get("DEER_URL"):
        url = push_config.get("DEER_URL")
    ...
    if len(response.get("content").get("result")) > 0:
        print("PushDeer2 推送成功！")
    else:
        print("PushDeer2 推送失败！错误信息：", response)
...
if push_config.get("DEER_KEY2"):
    notify_function.append(pushdeer2)
